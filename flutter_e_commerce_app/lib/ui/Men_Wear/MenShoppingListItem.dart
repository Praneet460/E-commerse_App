...
class MenShoppingListItem extends StatelessWidget {
  final Product product;

  const MenShoppingListItem({@required this.product});
  @override
  ...
  Widget build(BuildContext context) {
    return Material(
      elevation: 8.0,
        child: InkWell(
          onTap: () =>
        Navigator.of(context).push(new MaterialPageRoute(builder: (context) => MenDetailPage(product))),
          child: new Stack(
            fit: StackFit.expand,
            children: <Widget>[
              ListItemImage(product),
              ListItemRating(product),
